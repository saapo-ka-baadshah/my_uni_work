...
int main(int argc, char **argv)
{
    ...
    // std::cout << "Number of WorkItem Size (max): " << device.getInfo<CL_DEVICE_MAX_WORK_ITEM_SIZES>() << std::endl;

    // Create a command queue
    cl::CommandQueue queue(context, device, CL_QUEUE_PROFILING_ENABLE);

    // Load the source code
    cl::Program program = OpenCL::loadProgramSource(context, "../src/Mandelbrot.cl");
    ...
    // Declare some values
    std::size_t wgSizeX = 8; // Number of work items per work group in X direction
    std::size_t wgSizeY = 8;
    ...
    mandelbrotKernel.setArg<float>(2, xmin);
    mandelbrotKernel.setArg<float>(3, xmax);
    mandelbrotKernel.setArg<float>(4, ymin);
    mandelbrotKernel.setArg<float>(5, ymax);

    cl::Event execution;

    queue.enqueueNDRangeKernel(mandelbrotKernel,
                               cl::NullRange,
    ...
    //TODO
    Core::TimeSpan cpuTime = t2 - t1;
    Core::TimeSpan gpuTime = OpenCL::getElapsedTime(execution);
    Core::TimeSpan copyTime = OpenCL::getElapsedTime(copy2);
